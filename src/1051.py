...
if __name__ == "__main__":
    index=int(input())#输入测试组数
    results = []  # 用于存储所有组的输出
    for _ in range(index):#每一组
        dis=int(input())#输入终点距离
        num=int(input())#输入传感器数量
        Llis=[]#存放每个传感器范围左端点
        Rlis=[]#存放每个传感器范围右端点
        tlis=[]#存放每个传感器的传输时间
        for _ in range(num):
            zancun=[]
            zancun=list(map(int,input().split()))
            Llis.append(zancun[0])#列表下标为方括号
            Rlis.append(zancun[1])
            tlis.append(zancun[2])
